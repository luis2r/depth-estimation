...
def main():
    ...
    parser.add_argument('-t', '--type', required=True, choices=('txt'),
                        help='select the way RGB images are gathered, e.g. copy raw RGB images to '
                             ...
                             + 'images (\'txt\')')
    parser.add_argument('-o', '--out_txt_prefix', default='kitti_train_monodepth',
...
#    depth_search = "2011_*_*_drive_*_sync/proj_depth/groundtruth/image_0[2,3]/*.png"
    depth_search = "2011_*_*_drive_*_sync/proj_depth/groundtruth/image_02/*.png"
    depth_files = sorted(glob.glob(os.path.join(args.depth_map_base, depth_search)))
    print "len(depth_files) " +str(len(depth_files))
    num_depth_files = len(depth_files)
#    num_train_files = 85898/2
    num_train_files = 85898/2
#    num_val_files = 6852
    num_val_files = 6852/2
...
#    image_search = "2011_*_*/2011_*_*_drive_*_sync/image_0[2,3]/data/*.png"
    ...
    print "len(num_image_files) " +str(num_image_files)
    num_raw_image_files = 95778
#    num_raw_image_files = 95778/2
...
#    elif args.type == "txt":
    if args.type == "txt":
        ...
        txt_out_depth = args.out_txt_prefix + "_depth_maps_monodepth.txt"
        txt_out_imgs = args.out_txt_prefix + "_images_monodepth.txt"
#        txt_out_intrinsics = args.out_txt_prefix + "_intrinsics.txt"
        if (os.path.isfile(txt_out_depth) or os.path.isfile(txt_out_imgs)) \
           and not args.force_override:
            print "WARNING: Output text files {}, {} or ".format(txt_out_depth, txt_out_imgs) + "{} already exist. ".format(txt_out_intrinsics) + "Use -f option to override or -o to change output name."
            exit()
        txt_depth_files = []
        txt_images_files = []
        txt_images_files_right = []
#        txt_intrinsic_files = []
    else:
...
#                           and "/" + image + "/" in img_fn and "/" + frame in img_fn]
        matching_img_fn = [img_fn for img_fn in image_files if "/" + drive + "/" in img_fn
                           and "/" + "image_02" + "/" in img_fn and "/" + frame in img_fn]
        matching_img_fn_right = [img_fn_right for img_fn_right in image_files if "/" + drive + "/" in img_fn_right
                           and "/" + "image_03" + "/" in img_fn_right and "/" + frame in img_fn_right]
...
#                             if "/" + date + "/" + "calib_cam_to_cam.txt" in calib_fn]
        if len(matching_img_fn) <= 0:
            ...
            matching_img_fn_right = matching_img_fn_right[0]
...
#            txt_depth_files.append(os.path.abspath(depth_fn))
            ...
            txt_images_files_right.append(os.path.abspath(matching_img_fn_right))
...
#                f.write(depth_fn + "\n")
        with open(txt_out_imgs, "w") as f:
            for i in range(len(txt_images_files)):
                f.write(txt_images_files[i] +" "+ txt_images_files_right[i]+ "\n")
